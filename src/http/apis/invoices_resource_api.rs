...
pub struct CreateInvoicesResourceParams {
    /// payload
    pub invoice_payload: Option<crate::http::models::InvoicePayloadWrap>,
}
...
pub async fn create_invoices_resource(
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateInvoicesResourceError> =
            parse_json(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
            entity: local_var_entity,
...
pub async fn list_invoices_resource(
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        parse_json(&local_var_content).map_err(Error::from)
        // tracing::trace!("Response: {}", local_var_content); serde_json::from_str(&local_var_content).map_err(Error::from)
    } else {
